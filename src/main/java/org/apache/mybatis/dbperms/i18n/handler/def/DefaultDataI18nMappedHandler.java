 ...
 * the License.
 */
package org.apache.mybatis.dbperms.i18n.handler.def;
...
import org.apache.mybatis.dbperms.i18n.handler.DataI18nMappedHandler;
import org.apache.mybatis.dbperms.i18n.handler.DataI18nMapper;
...
public class DefaultDataI18nMappedHandler implements DataI18nMappedHandler {
	...
	@Override
	public String getPrimaryName(PermPrimary i18nPrimary, Object source) throws Exception{
		...
		synchronized (source) {
			...
			if(StringUtils.isEmpty(primaryName)){
				for (Class<?> superClass = clazz; superClass != Object.class && superClass != null; superClass = superClass.getSuperclass()) {
					for (Field field : superClass.getDeclaredFields()) {
						//查找对象属性上的主键注解，直到找到为止
						if(field.getAnnotation(PermPrimary.class) != null){
							ret = field.getName();
	...
	@Override
	public DataI18nMapper handle(Locale locale,PermMapper i18nMapper, String primaryName , Object orginObject, Object i18nObject) throws Exception {
		...
		//原查询行数据对象不是Map
			...
			for (PropertyDescriptor propDes : orginDescriptors) {
				...
				for (Class<?> superClass = i18nObject.getClass(); superClass != Object.class && superClass != null; superClass = superClass.getSuperclass()) {
					...
					//查找对象属性上的映射注解
					PermColumn i18nColumn = field.getAnnotation(PermColumn.class);
					if(i18nColumn != null){
						//默认以当前指定的列为映射列
		...
		//2、解析方法注解映射关系
		PermColumn[] i18nColumns  = i18nMapper.value();
